...
function getHours() {
  ...
  return now2.getHours();
}
function isInRange(hour, from, to) {
  if (from < to) {
...
function darkliteInit() {
  ...
  let saveUserTheme = localStorage.getItem("fls-user-theme");
  let userTheme;
  if (document.querySelector("[data-fls-darklite-time]")) {
    ...
    const [timeFrom, timeTo] = customRange.split(",").map((n) => +n);
    userTheme = isInRange(getHours(), timeFrom, timeTo) ? "dark" : "light";
  } else {
  ...
  if (resetButton) {
    resetButton.addEventListener("click", function() {
      localStorage.removeItem("fls-user-theme");
      saveUserTheme = null;
      htmlBlock.removeAttribute("data-fls-darklite-dark");
      htmlBlock.setAttribute("data-fls-darklite-light", "");
    });
  }
  function setThemeClass() {
    const theme = saveUserTheme || userTheme || "light";
    htmlBlock.removeAttribute("data-fls-darklite-light");
    htmlBlock.removeAttribute("data-fls-darklite-dark");
    htmlBlock.setAttribute(`data-fls-darklite-${theme}`, "");
  }
  function changeTheme(saveTheme = false) {
    ...
    if (saveTheme) {
      localStorage.setItem("fls-user-theme", newTheme);
...
function initSliders() {
  if (document.querySelector(".selected-work__slider")) {
    new Swiper(".selected-work__slider", {
